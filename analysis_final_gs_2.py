    ...
    for mutm,mutp in itertools.product(m_mut_freq_dict.keys(),p_mut_freq_dict.keys()):
        ...
        if correlation==None: 
            ...
            if mutm in m_freq_muts:
                if mutp in p_f_mut_freq_dict.keys(): 
                    freq_ILDS.append(0)
            #print(mut_ILD_dict_m)
            #sys.exit()
            #mut_ILD_dict_m[key]=dict_list
            #print(dict_list)
...
def ILD_stats(m_space_dict,p_space_dict,mut_dict_m,mut_dict_p,moth_demes,plant_demes,s_f,sigma,rep): 
    ...
    for m,p in itertools.product(mdict,pdict): 
        if mdict[m]==pdict[p]:
            ...
            if genom==True and rep==1: ## toggle genomic analysis
                ...
                for i,my_list in enumerate(mut_list): 
                    target=d_dict_m[str(sigma_m)][i]
                    target.append(my_list)
            ...
            if z==True and rep==1: 
                ...
                if z_save==True: 
                    for i,my_list in enumerate(output):
                        target=z_dict[str(sigma_m)][i]
                        target.append(my_list)
                
                print('z analysis done! ')
            if ILD==True and rep==1:
                print(rep)
                mut_stuff=ILD_stats(space_dict_m,space_dict_p,mut_dict_m,mut_dict_p,moth_demes,plant_demes,0.1,sigma_m,rep)
                # ^ compute stats for ILD
                if ILD_save==True: 
                    for i,my_list in enumerate(mut_stuff):
