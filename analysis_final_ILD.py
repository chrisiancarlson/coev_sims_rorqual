    ...
    for mut in mut_dict: 
        ...
        for xpos in range(1,big+1): 
            ...
            for ypos in range(1,big+1): 
                ...
                # set counters
                s_num=0# number of chromosomes in subpop
                s_mut_num=0# number of chromosomes with mutation of interest in sub pop
                ...
                for ind in ind_list: 
                    ...
                    if pos[0]<xpos and pos[0]>=xpos-delta and pos[1]<ypos and pos[1]>=ypos-delta: 
                        ...
                        if local_num==1: 
                            heterozygotes+=1     
                        s_mut_num+=local_num
                        ind_list.remove(ind) # remove the individual from the key 
                        # list to make the search more efficient
...
def sliding_window_ILD(mut_dict_m,moth_demes,m_mut_freq_dict,p_mut_freq_dict,s_f,p_f_mut_freq_dict,space_dict_m): 
    ILDS=[]
    freq_ILDS=[]
    mut_ILD_dict_m={}
    # calculate ILDS: 
    for key in m_mut_freq_dict.keys(): mut_ILD_dict_m[key]=[]
    m_freq_muts=[]
    for mut in m_mut_freq_dict.keys(): 
        counter =0
        for deme in m_mut_freq_dict[mut]: 
            if deme>0: 
                counter+=1 
        if counter/len(moth_demes)>s_f: 
            m_freq_muts.append(mut)
    ...
    for mutm,mutp in itertools.product(m_mut_freq_dict.keys(),p_mut_freq_dict.keys()):
        ...
        if correlation !=None:
            ...
            if mutm in m_freq_muts:
                if mutp in p_f_mut_freq_dict.keys(): 
                    freq_ILDS.append(correlation)
            mut_ILD_dict_m[mutm].append(np.abs(correlation))
        if correlation==None: 
            mut_ILD_dict_m[mutm].append(0)
            if mutm in m_freq_muts:
                if mutp in p_f_mut_freq_dict.keys(): 
                    freq_ILDS.append(0)
    
    print("onto sliding window: ")
    # Define a window in the genome: 
    val=10
    delta=100000/val
    windows=np.linspace(delta-1,99999,val)
    window_muts=[[] for window in windows]
    window_dict={}
    freq_window_dict={}
    keys_list = list(mut_ILD_dict_m.keys())
    for i,max in enumerate(windows): 
        ILDS_in_window=[]
        freq_ILDS_in_window=[]
        w_freq_effects=0
        w_effects=0
        for mut in keys_list: 
            pos = float(mut_dict_m[mut][3])
            # if a mutation is within a given window: 
            if pos<max and pos>=max-delta: 
                # add up the sum of the effect of every mutation in a window
                #w_abs_effects+=np.abs(float(mut_dict_m[mut][0]))
                w_effects+=float(mut_dict_m[mut][0])
                m_ILDS=mut_ILD_dict_m[mut]
                ILDS_in_window.append(m_ILDS)
                if mut in m_freq_muts: 
                    freq_ILDS_in_window.append(m_ILDS)
                    w_freq_effects+=float(mut_dict_m[mut][0])
                window_muts[i].append(mut)
                keys_list.remove(mut) # remove from list to make less computationally intensive
        # calculate every pairwise correlation between ILDS in each window 
        if len(ILDS_in_window)>1:
            correlations=[]
            for i,ilds in enumerate(ILDS_in_window): 
                for j,ilds_2 in enumerate(ILDS_in_window): 
                    if i!=j: 
                        correlation=calculate_correlation(ilds,ilds_2)
                        if correlation !=None: correlations.append(np.abs(correlation))
                        else: 
                            correlations.append(0) 
                            
            window_dict[max]=[np.mean(correlations),np.abs(w_effects)]

        if len(freq_ILDS_in_window)>1:
            correlations=[]

            for i,ilds in enumerate(freq_ILDS_in_window): 
                for j,ilds_2 in enumerate(freq_ILDS_in_window): 
                    if i!=j: 
                        correlation=calculate_correlation(ilds,ilds_2)
                        if correlation !=None: correlations.append(correlation)
                        else: 
                            correlations.append(0)
            freq_window_dict[max]=[np.mean(correlations),np.abs(w_freq_effects)]
    
    if False: 
        # flatten list of demes: 
        all_moths = [val for sublist in moth_demes for val in sublist]
        num=2*len(all_moths)
        mut_sample_frequencies={}
        # calculate frequency of each mutation in sample: 
        for mut_list in window_muts: 
            for mut in mut_list: 
                mut_prop=0
                for moth in all_moths: 
                    chrom1=space_dict_m[moth][2][0]
                    chrom2=space_dict_m[moth][2][1]
                    if mut in chrom1: mut_prop+=1/num
                    if mut in chrom2: mut_prop+=1/num
                #print(mut_prop)
                mut_sample_frequencies[mut]=mut_prop
        # calculate mean pairwise r^2 for each mutation in window
        
        for window,mut_list in zip(windows, window_muts): 
            r2s=[]
            freqr2s=[]
            for  mut1,mut2 in itertools.combinations(mut_list, 2): 
                p1=mut_sample_frequencies[mut1]
                p2=mut_sample_frequencies[mut2]
                p12=0
                for moth in all_moths: 
                    chrom1=space_dict_m[moth][2][0]
                    chrom2=space_dict_m[moth][2][1]
                    if mut1  in chrom1: 
                        if mut2 in chrom1: p12+=1/num
                    if mut1 in chrom2: 
                        if mut2 in chrom2: p12+=1/num
                #print(p12)
                
                r2=(p12-p1*p2)**2/(p1*(1-p1)*p2*(1-p2))
                
                r2s.append(r2)    
                if mut1 in m_freq_muts: 
                    if mut2 in m_freq_muts:
                        freqr2s.append(r2)

            freq_window_dict[window].append(np.mean(freqr2s))
            window_dict[window].append(np.mean(r2s))
    ...
    if False: 
        ...
        delta=100000/val
        windows=np.linspace(delta-1,99999,val)
        for deme in moth_demes: 
            if len(deme)<2: 
                continue
            muts=[]
            for ind in deme: 
                for chrom in space_dict_m[ind][2]: 
                    for mut in chrom: 
                        if mut in m_freq_muts:
                            muts.append(mut)
            unique_deme_muts=list(set(muts))
            #print(unique_deme_muts)
            window_corrs=[]
            window_effects=[]
            for i,max in enumerate(windows): 
                ...
                for mut in unique_deme_muts: 
                    ...
                    if pos<max and pos>=max-delta: 
                        ...
                        ILDS_in_window.append(m_ILDS)
                        window_muts[i].append(mut)
                        unique_deme_muts.remove(mut)
                if len(ILDS_in_window)>1:
                    #print('nice')
                    flag=True
                    # calculate every pairwise correlation between ILDS in each window 
                    correlations=[]
                    for j,ilds in enumerate(ILDS_in_window): 
                        for z,ilds_2 in enumerate(ILDS_in_window): 
                            if j!=z: 
                                correlation=calculate_correlation(ilds,ilds_2)
                                if correlation !=None: correlations.append(np.abs(correlation))
                ...
                if flag==True:
                    window_corr=np.mean(correlations)
                    effect=abs(w_effects)
                    window_corrs.append(window_corr)
                    window_effects.append(effect)
            #print(window_corrs)
            #print(window_effects)
            deme_corrs.append(calculate_correlation(window_corrs,window_effects))
            
                    
            # mean_corr=np.mean(correlations)
            # window_corrs.append(mean_corr)
            # window_effects.append(abs(window_effects))
            # deme_corr=calculate_correlation(window_corrs,window_effects)
            # deme_corrs.append(deme_corr)
        print(deme_corrs)
        


    return ILDS, freq_ILDS,window_dict,freq_window_dict
...
def ILD_stats(m_space_dict,p_space_dict,mut_dict_m,mut_dict_p,s_f,sigma,rep,num): 
    # positions
    positions=random_sampler(m_space_dict,p_space_dict,sigma,num)
    # list of individuals in demes
    moth_demes=find_individuals(positions,m_space_dict,sigma)
    plant_demes=find_individuals(positions,p_space_dict,sigma)
    ...
    # sub sample to half of the mut_freq_dicts to make the ILD calculation less computationally intensive:
    m_dict_size=int(len(m_mut_freq_dict)/2)
    p_dict_size=int(len(p_mut_freq_dict)/2)

    m_s_mut_freq_dict = {k: m_mut_freq_dict[k] for k in random.sample(list(m_mut_freq_dict), m_dict_size)}
    p_s_mut_freq_dict = {k: p_mut_freq_dict[k] for k in random.sample(list(p_mut_freq_dict), p_dict_size)}

    # subsample the frequent mutation dictionary from the full mutation frequency dictionary to make the ILD calculation less computationally intensive:
    for mut in list(m_f_mut_freq_dict.keys()): 
        if mut not in m_s_mut_freq_dict.keys(): 
            del m_f_mut_freq_dict[mut]
    
    for mut in list(p_f_mut_freq_dict.keys()): 
        if mut not in p_s_mut_freq_dict.keys(): 
            del p_f_mut_freq_dict[mut]

    m_afs,m_freq_muts=s_afs(m_s_mut_freq_dict,len(moth_demes),s_f)
    p_afs,p_freq_muts=s_afs(p_s_mut_freq_dict,len(plant_demes),s_f)
   #print(m_freq_muts)
   #print(p_freq_muts)
    ...
    m_ef_tot,m_times_tot=mutation_effects_time(m_s_mut_freq_dict.keys(),mut_dict_m)
    p_ef_tot,p_times_tot=mutation_effects_time(p_s_mut_freq_dict.keys(),mut_dict_p)

    #print('onto ILDS')
    if False:# rep<2: 
        ILDS,freq_ILDS,window_dict,freq_window_dict=sliding_window_ILD(mut_dict_m,moth_demes,m_mut_freq_dict,p_mut_freq_dict,s_f,p_f_mut_freq_dict,m_space_dict)
        
        with open('window_dict_d_{}_rep_{}.json'.format(str(sigma),str(rep)), 'w') as json_file:
                json.dump(window_dict, json_file)
        
        print('hooray {}'.format(sigma))

        with open('f_window_dict_d_{}_rep_{}.json'.format(str(sigma),str(rep)), 'w') as json_file:
                json.dump(freq_window_dict, json_file)
    
    else: 
        ILDS=[]
        freq_ILDS=[]
        for mutm,mutp in itertools.product(m_s_mut_freq_dict.keys(),p_s_mut_freq_dict.keys()):
            correlation = calculate_correlation(m_s_mut_freq_dict[mutm], p_s_mut_freq_dict[mutp])
            if correlation !=None:
                ILDS.append(correlation)
                if mutm in m_f_mut_freq_dict.keys():
                    if mutp in p_f_mut_freq_dict.keys(): 
                        freq_ILDS.append(correlation)
            if correlation==None: 
                ILDS.append(0)
                if mutm in m_f_mut_freq_dict.keys():
                    if mutp in p_f_mut_freq_dict.keys(): 
                        freq_ILDS.append(correlation)
    # moth effects, plant effects, frequent moth effects, frequent plant effects, moth spatial freq, plant spatial freq, moth times, plant times, frequent moth times, frequent plant times, ILdS, frequent ILDS
...
def r2_decline(mut_dict_m,space_dict_m): 
    all_moths=list(space_dict_m.keys())
    num=2*len(all_moths) # number of chromosomes
    mut_list=[]
    # mutations must be found in 1% of individuals: 
    cutoff=int(num*0.05)# 1% of chromosomes
    for mut in mut_dict_m.keys():
        number=float(mut_dict_m[mut][2])
        if number>cutoff: 
            mut_list.append(mut)
    
    overall_matrix=[]
    
    for mut_A in mut_list: 
        distance_dict={}
        pos_A = mut_dict_m[mut_A][3] # position of mutation in genome
        #print(pos_A)
        for mut_B in mut_list: 
            pos_B=mut_dict_m[mut_B][3]
            distance=abs(float(pos_A)-float(pos_B))
            distance_dict[distance]=mut_B
        # sort by ascending genomic distance
        sorted_distance_dict = dict(sorted(distance_dict.items(), key=lambda item: item[0])) 
        total_mut_num=0
        mean_r_2=0
        p_A=float(mut_dict_m[mut_A][2])/num
        # if p_A>1: 
        #     print('whoops')
        #     print(mut_dict_m[mut_A][2])
        #     print(p_A)
        #     sys.exit()
        # b_max=0
        # p_AB_max=0
        for distance in sorted_distance_dict.keys(): 
            total_mut_num+=1# add another mutation to total muts in range
            mut_B=sorted_distance_dict[distance]# second mutation
            p_B=float(mut_dict_m[mut_B][2])/num # frequency of second mutation
            #print(p_B)
            # if p_B>b_max: 
            #     b_max=p_B
            
            p_AB=0# frequency of each haplotype
            p_aB=0
            p_Ab=0
            p_ab=0
            for moth in all_moths: 
                chroms=space_dict_m[moth][2]
                for chrom in chroms: 
                    if mut_A  in chrom: 
                        if mut_B in chrom: p_AB+=1/num
                        else: p_Ab+=1/num
                    else: 
                        if mut_B in chrom: p_aB+=1/num
                        else: p_ab+=1/num
            ...
            #print(p_AB+p_ab+p_Ab+p_aB)
            
            r2=(p_AB*p_ab-p_Ab*p_aB)**2/(p_A*(1-p_A)*p_B*(1-p_B))
            #r22=(p_AB*-p_A*p_B)**2/(p_A*(1-p_A)*p_B*(1-p_B))
            #print(r2)
            #new_mean=(mean_r_2*(total_mut_num-1)+r2)/total_mut_num
            #mean_r_2=new_mean
            overall_matrix.append([distance,r2])
        # print(b_max)
        # print(p_AB_max)
...
def run_all_analysis():
    ...
    for m,p in itertools.product(mdict,pdict): 
        if mdict[m]==pdict[p]:
            sigma_p,sigma_m,num_p,rep=pdict[p]
            
            #sigma_p,sigma_m,num_p,rep=mdict[m][0]
            #test=str(sigma_m)+str(rep)
            #if test not in rep_list:
            print('rep {}'.format(str(rep))) 
            ...
            if z==True: 
                ...
                num=20
                output,moth_demes,plant_demes=phenotypic_analysis(space_dict_m,mut_dict_m,space_dict_p,mut_dict_p,2*float(sigma_m),num)
            ...
            if ILD==True:
                ...
                mut_stuff=ILD_stats(space_dict_m,space_dict_p,mut_dict_m,mut_dict_p,0.1,sigma_m,rep,200)
            ...
            if r2==True and rep<=3: 
                #print('rep ={}, sigma = {}'.format(rep,sigma_m))
                space_dict_m,mut_dict_m = import_output_full("./{}".format(m))
                print('imported, start timer')
                overall_matrix=r2_decline(mut_dict_m,space_dict_m)
                write_my_csv("r2_decline_d_{}_rep_{}.csv".format(sigma_m,rep),overall_matrix)
